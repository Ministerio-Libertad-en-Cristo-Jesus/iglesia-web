...
import { updateTaskStatusUser } from "@/app/lib/actions/updateTaskStatusUser"
interface Props {
  id: string
  author: string
  title: string
...
const CardTask = ({ author, title, importance, status, description, id, idOwner }: Props) => {
  ...
  return (
    <article className="flex flex-col items-start w-full rounded-xl bg-white border-2 border-blueI p-4 gap-4 md:gap-2">
      ...
      <form action={updateTaskStatusUser}>
        <input id="id-owner" className="hidden" type="text" name="idOwner" value={JSON.parse(JSON.stringify(idOwner))} readOnly />
